...
def test_pass_with_head_sha_records_it_on_the_event(tmp_path):
    # Added for #37's merge_policy attestation binding (Review Task #111).
    store = Store(tmp_path / "state.db")
    task = _make_task()
    head_sha = "a" * 40

    request_review(task, AgentName.CODEX, AgentName.CLAUDE, passed=True, store=store, head_sha=head_sha)

    events = query_events(store, event_types=[EventType.REVIEW_PASSED])
    assert events[0]["payload"]["head_sha"] == head_sha
    store.close()


def test_pass_without_head_sha_omits_it_from_the_event(tmp_path):
    store = Store(tmp_path / "state.db")
    task = _make_task()
    ...
    events = query_events(store, event_types=[EventType.REVIEW_PASSED])
    assert "head_sha" not in events[0]["payload"]
    store.close()

...
- Before start and at/after cutoff, new PLANNED tasks become NEXT_CYCLE. Inside
  ...
  otherwise they stay PLANNED for reconsideration. Planner BLOCKED/NEEDS_LUCAS tasks stay blocked. Existing
...
- At cycle start every NEXT_CYCLE task is evaluated, with no batch cap. A pending,
  missing, self or cyclic dependency leaves the task in NEXT_CYCLE. Only DONE
  satisfies a dependency. Explicit BLOCKED/NEEDS_LUCAS states are never cleared.
